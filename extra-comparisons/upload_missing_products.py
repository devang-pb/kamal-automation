#!/usr/bin/env python3
"""Upload extra_merged_comparisons.csv to the Procwise missing-products endpoint."""

import csv
import logging
import os
import sys
from pathlib import Path

import requests
from dotenv import load_dotenv

load_dotenv()

BASE_URL = "https://procwise.purpleblock.ai"
WAREHOUSE_ID = "866d1abb-de61-45f9-b8c6-dc71d82e7501"
CSV_PATH = Path(os.environ.get("OUTPUT_DIR", "output")) / "extra_merged_comparisons.csv"

COLUMN_MAP = {
    "ean": "ean",
    "name": "productName",
    "cosmetic": "cosmetic",
    "productos": "productos",
    "lodoro": "lodoro",
    "multimarcas": "multimarcas",
    "elite": "elitePerfumes",
    "yauras": "yauras",
    "lattafa": "lattafa",
    "sairam": "sairam",
    "paris": "paris",
}

logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s  %(levelname)-8s  %(message)s",
)
log = logging.getLogger(__name__)


def login(session: requests.Session) -> None:
    email = os.getenv("PROCWISE_EMAIL")
    password = os.getenv("PROCWISE_PASSWORD")
    if not email or not password:
        log.error("PROCWISE_EMAIL / PROCWISE_PASSWORD not set in .env")
        sys.exit(1)

    resp = session.post(
        f"{BASE_URL}/api/auth/login",
        json={"email": email, "password": password},
    )
    resp.raise_for_status()
    data = resp.json()
    if not data.get("success"):
        log.error("Login failed: %s", data)
        sys.exit(1)
    log.info("Logged in (userId: %s)", data["userId"])


def build_items(rows: list[dict]) -> list[dict]:
    items = []
    for row in rows:
        item = {}
        for csv_col, payload_key in COLUMN_MAP.items():
            item[payload_key] = row.get(csv_col, "").strip() or "-"
        items.append(item)
    return items


def upload_missing_products(session: requests.Session, items: list[dict]) -> None:
    resp = session.post(
        f"{BASE_URL}/api/retail-intel/missing-products",
        json=items,
        headers={"X-Warehouse-Id": WAREHOUSE_ID},
    )
    resp.raise_for_status()
    data = resp.json()
    if data.get("success"):
        log.info("Upload successful: %s", data["message"])
    else:
        log.error("Upload failed: %s", data)
        sys.exit(1)


def main() -> None:
    if not CSV_PATH.exists():
        log.error("CSV not found: %s", CSV_PATH)
        sys.exit(1)

    with open(CSV_PATH, newline="", encoding="utf-8") as f:
        rows = list(csv.DictReader(f))
    log.info("Read %d items from %s", len(rows), CSV_PATH.name)

    items = build_items(rows)

    session = requests.Session()
    login(session)
    upload_missing_products(session, items)


if __name__ == "__main__":
    main()
