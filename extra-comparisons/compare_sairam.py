...
CATALOG_FILE = os.path.join(os.environ.get("OUTPUT_DIR", "output"), "scrape_sairam.csv")
OUTPUT_FILE = os.path.join(os.environ.get("OUTPUT_DIR", "output"), "compare_sairam.csv")
...
def load_master_rows(path: str) -> list[MasterProduct]:
    ...
    with open(path, "r", encoding="utf-8-sig") as f:
        ...
        if not barcode_key or not name_key:
            raise ValueError(
                f"{path} must include barcode and name columns. Found: {fieldnames}"
            )
