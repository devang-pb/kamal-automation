#!/usr/bin/env python3
"""Generate catalog_costanera.csv directly from BSale API.

Fetches only Costanera-office stock with base price list pricing.
"""
...
def load_costs_by_ean() -> dict:
    """Load EAN -> avg_cost mapping from costs.csv if it exists."""
...
def main():
    ...
    # 1. Stocks: server-side filtered to Costanera office only
    raw_stocks = client.get_stocks_by_office(COSTANERA_OFFICE_ID)
    costanera_stock = {}
    for s in raw_stocks:
        ...
        if qty > 0:
            costanera_stock[vid] = int(qty)
    logger.info(f"Costanera in-stock variants: {len(costanera_stock)}")
    ...
    # 5. Join: only variants in stock at Costanera AND with a base price
    fieldnames = ["ean", "sku", "name", "stock", "price"]
    if ean_costs:
        fieldnames.append("avg_cost")

    rows = []
    for vid, qty in costanera_stock.items():
    ...
    catalog_path = OUTPUT_DIR / "catalog_costanera.csv"
