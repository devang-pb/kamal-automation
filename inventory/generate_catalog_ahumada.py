#!/usr/bin/env python3
"""Generate catalog_ahumada.csv directly from BSale API.

Fetches only Ahumada-office stock with base price list pricing.
"""
...
def load_costs_by_ean() -> dict:
    """Load EAN -> avg_cost mapping from costs_ahumada.csv if it exists."""
    costs_path = OUTPUT_DIR / "costs_ahumada.csv"
...
def main():
    ...
    # 1. Stocks: server-side filtered to Ahumada office only
    raw_stocks = client.get_stocks_by_office(AHUMADA_OFFICE_ID)
    ahumada_stock = {}
    for s in raw_stocks:
        ...
        if qty > 0:
            ahumada_stock[vid] = int(qty)
    logger.info(f"Ahumada in-stock variants: {len(ahumada_stock)}")
    ...
    # 4. Costs: load from costs_ahumada.csv (produced by pull_costs_ahumada.py)
    ean_costs = load_costs_by_ean()
    if ean_costs:
        logger.info(f"Loaded {len(ean_costs)} costs from costs_ahumada.csv")
    else:
        logger.info("No costs_ahumada.csv found — run pull_costs_ahumada.py first to include avg_cost")

    # 5. Join: only variants in stock at Ahumada AND with a base price
    fieldnames = ["ean", "sku", "name", "stock", "price"]
    if ean_costs:
        fieldnames.append("avg_cost")

    rows = []
    for vid, qty in ahumada_stock.items():
    ...
    catalog_path = OUTPUT_DIR / "catalog_ahumada.csv"
