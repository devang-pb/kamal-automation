#!/usr/bin/env python3
"""Upload catalog.csv inventory to the Procwise platform."""
...
CSV_PATH = Path(os.environ.get("OUTPUT_DIR", Path(__file__).parent / "output")) / "catalog.csv"
CSV_COLUMNS = ["ean", "sku", "name", "stock", "price", "avg_cost", "brand"]
...
def upload_inventory(session: requests.Session, items: list[dict]) -> None:
    resp = session.post(f"{BASE_URL}/api/inventory", json=items)
    resp.raise_for_status()
    data = resp.json()
    if data.get("success"):
...
def main() -> InventoryDiff | None:
    ...
    # Fetch current inventory and compute diff before uploading
    try:
        old_items = fetch_current_inventory(session)
        diff = compute_diff(old_items, items, warehouse="Gorbea")
