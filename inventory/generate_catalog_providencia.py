#!/usr/bin/env python3
"""Generate catalog_providencia.csv directly from BSale API.

Fetches only Providencia-office stock with base price list pricing.
"""
...
def load_costs_by_ean() -> dict:
    """Load EAN -> avg_cost mapping from costs_providencia.csv if it exists."""
    costs_path = OUTPUT_DIR / "costs_providencia.csv"
...
def main():
    ...
    # 1. Stocks: server-side filtered to Providencia office only
    raw_stocks = client.get_stocks_by_office(PROVIDENCIA_OFFICE_ID)
    providencia_stock = {}
    for s in raw_stocks:
        ...
        if qty > 0:
            providencia_stock[vid] = int(qty)
    logger.info(f"Providencia in-stock variants: {len(providencia_stock)}")
    ...
    # 4. Costs: load from costs_providencia.csv (produced by pull_costs_providencia.py)
    ean_costs = load_costs_by_ean()
    if ean_costs:
        logger.info(f"Loaded {len(ean_costs)} costs from costs_providencia.csv")
    else:
        logger.info("No costs_providencia.csv found — run pull_costs_providencia.py first to include avg_cost")

    # 5. Join: only variants in stock at Providencia AND with a base price
    fieldnames = ["ean", "sku", "name", "stock", "price"]
    if ean_costs:
        fieldnames.append("avg_cost")

    rows = []
    for vid, qty in providencia_stock.items():
        ...
        if ean_costs:
            cost = ean_costs.get(info["ean"])
            row["avg_cost"] = cost if cost is not None else ""
        rows.append(row)
    ...
    catalog_path = OUTPUT_DIR / "catalog_providencia.csv"
