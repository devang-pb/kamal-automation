#!/usr/bin/env python3
"""Pull average unit costs from BSale's stock report endpoint.
...
Outputs: output/costs.csv  (ean, avg_cost, brand)
...
def extract_row_data(row):
    """Extract ean, avg_cost, and brand from a bodyRow's fields."""
    ...
    for field in fields:
        if field.get("hash") == "unit_cost":
            ...
            if isinstance(value, dict):
                ...
                brand = str(inner.get("variantDescription", "")).strip()
                if brand == ".":
                    brand = ""
...
def main():
    ...
    with open(costs_path, "w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["ean", "avg_cost", "brand"])
        writer.writeheader()
