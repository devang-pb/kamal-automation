...
SITE_NAME = "Productos de Lujo"
SITE_URL = "https://productosdelujo.cl"
OUTPUT_FILE = os.path.join(os.environ.get("OUTPUT_DIR", "output"), "compare_productos.csv")
...
def read_master(path: str):
    ...
    with open(path, "r", encoding="utf-8") as f:
        ...
        if not bc_key or not name_key or not price_key:
            raise ValueError(
                f"MASTER_FILE must have columns Bar Code/Name/Price (or equivalents). Found: {fieldnames}"
            )
        ...
        for r in reader:
            ...
            price_raw = str(r[price_key]).strip()
